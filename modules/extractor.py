...
def extract_yake_candidates(
    ...
    try:
        keywords = get_yake_extractor().extract_keywords(text[:MAX_MODEL_TEXT_CHARS])
    except Exception:
        return []
...
def process_competitors(
    ...
    for index, competitor in enumerate(competitors):
        ...
        texts.append(text[:MAX_MODEL_TEXT_CHARS])
        processed.append(item)

    docs = list(get_nlp().pipe(texts, batch_size=8))
